...
def covariance_inner(z):
    '''
    calculates the covariance matrix using inner products and returns the matrix
...
def covariance_outer(z):
    '''
    calculates the covariance matrix using outer products and returns the matrix
    z is the centered data
    '''
    n = np.shape(z)[0]
...
def cosine_dist(z):
    '''
    finds the correlation coefficient for each attribute and returns the correlation matrix
    z is the centered data
    '''
    n = np.shape(z)[0]
    norms = np.empty([n,1])
